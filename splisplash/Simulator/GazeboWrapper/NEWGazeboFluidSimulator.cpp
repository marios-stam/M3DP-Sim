...
void FluidSimulator::RunStep()
{
    ...
    /*if (simulationSteps % 1000 == 0) {
            ...
            const string path="/home/jason/fluid-engine-dev/build/bin/hybrid_liquid_sim_output/SPlisHSPlasH_out/"+fileName;   //determine where you
       are saving the files in
        ...
        string command="'/home/jason/fluid-engine-dev/build/bin/particles2obj' -i '"+path+"' -r 200,200,100 -k 0.05 -m spherical -o
       '/home/jason/fluid-engine-dev/build/bin/hybrid_liquid_sim_output/louis_test_"+std::to_string(simulationSteps)+".obj'";
...
void FluidSimulator::ParseSDF()
{
    ...
    for (physics::Model_V::iterator currentModel = models.begin(); currentModel != models.end(); ++currentModel)
    {
        ...
        // endeff link only
        std::string endeffLink = model_links;
        if (endeffLink == "gripper_link")
            ;
        {
                ...
                for (physics::Collision_V::iterator collision_it = collisions.begin(); collision_it != collisions.end(); ++collision_it)
                {
                    ...
                    else if (geometry_type == "plane")
                    {
                        ...
                        if ((*collision_it)->GetName() == "collision_ground_plane")
                        {
                            ...
                            size = ignition::math::Vector2d(
                                4.0, 4.0); //= geom_elem->GetElement(geom_type)->GetElement("size")->Get<ignition::math::Vector2d>();
                        }
                        else
                        {
