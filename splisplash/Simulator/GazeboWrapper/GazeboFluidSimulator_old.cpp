...
void FluidSimulator::RunStep()
{
    ...
    if (simulationSteps % 5 == 0)
    {
        ...
        for (unsigned int i = 0; i < model->numActiveParticles(); ++i)
        {
            gazebo::msgs::Set(fluid_positions_msg.add_position(),
                              ignition::math::Vector3d(model->getPosition(i)[0], model->getPosition(i)[1], model->getPosition(i)[2]));
        }
    ...
    for (int i = 0; i < SPH::Simulation::getCurrent()->numberOfBoundaryModels(); ++i)
    {
        ...
        {
            ignition::math::Vector3d boundary_particles =
                ignition::math::Vector3d(bm->getPosition(j)[0], bm->getPosition(j)[1], bm->getPosition(j)[2]);
    ...
    //	 	const string path="/home/marios/fluid-engine-dev/build/bin/hybrid_liquid_sim_output/SPlisHSPlasH_out/"+fileName;   //determine where you
    // are saving the files in
    //
    ...
    //	 	}
    //
    //  	string command="'/home/marios/fluid-engine-dev/build/bin/particles2obj' -i '"+path+"' -r 200,200,100 -k 0.05 -m spherical -o
    //  '/home/marios/fluid-engine-dev/build/bin/hybrid_liquid_sim_output/louis_test_"+std::to_string(simulationSteps)+".obj'";
    //
    //	 	outFile.close();
...
void FluidSimulator::ImportModel()
{
    ...
    myfile << "<geometry>\n";
    myfile << "<mesh><uri>/home/marios/fluid-engine-dev/build/bin/hybrid_liquid_sim_output/louis_test_" + std::to_string(simulationSteps) +
                  ".obj</uri></mesh>\n";
    myfile << "</geometry>\n";
    myfile << "</visual>\n";
    myfile << "</link>\n";
...
void FluidSimulator::RemoveModel() { system("gz model -m mymesh -d"); }
...
void FluidSimulator::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
    ...
    this->connections.push_back(event::Events::ConnectWorldUpdateEnd(boost::bind(&FluidSimulator::RunStep, this)));
}
        ...
        for (physics::Collision_V::iterator collision_it = collisions.begin(); collision_it != collisions.end(); ++collision_it)
        {
            ...
            else if (geometry_type == "plane")
            {
                ...
                if ((*collision_it)->GetName() == "collision_ground_plane")
                {
                    ...
                    size =
                        ignition::math::Vector2d(4.0, 4.0); //= geom_elem->GetElement(geom_type)->GetElement("size")->Get<ignition::math::Vector2d>();
                }
                else
                {
                ...
                common::MeshManager::Instance()->CreatePlane((*collision_it)->GetName(), ignition::math::Vector3d(0.0, 0.0, 1.0), 0.0, size,
                                                             ignition::math::Vector2d(4.0, 4.0), ignition::math::Vector2d());
...
void FluidSimulator::initBoundaryData()
{
    ...
    for (unsigned int i = 0; i < scene.boundaryModels.size(); ++i)
    {
        ...
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
        {
            ...
            sampling.sampleMesh(geo.numVertices(), geo.getVertices().data(), geo.numFaces(), geo.getFaces().data(), scene.particleRadius, 10, 1,
                                boundaryParticles);
            STOP_TIMING_AVG;
